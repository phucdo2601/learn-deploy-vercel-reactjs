...
export default function ProductDetailsSummary({ cart, product, onAddCart, onGotoStep, ...other } : any) {
  ...
  const isMaxQuantity = cart.filter((item: any) => item.id === id).map((item: any) => item.quantity)[0] >= available;
...
function Incrementer({ name, available, quantity, onIncrementQuantity, onDecrementQuantity }: any) {
  return (
    <Box
      sx={{
        py: 0.5,
